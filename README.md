...
| 角色 | 默认职责 |
| --- | --- |
| 超级管理员 | 用户管理、权限配置、系统配置、全量任务、打款跟踪、仪表盘、日志 |
| 子管理员 | 仪表盘、全量任务、分数审核、全店铺打款数据等管理能力，具体以权限配置为准 |
| 运营 | 发布美工任务、发布运营助理任务、审核自己发布的作品/任务、开启打款、查看同店铺记录 |
| 客服 | 发布基础美工任务、审核基础美工作品、修改/重开客服基础美工任务 |
| 美工设计师 | 任务大厅接单、待做任务、待审核任务、上传作品、个人统计 |
| 运营助理 | 按所属店铺参与任务协作、提交完成凭证或完成次数、个人统计 |
...
- 打款跟踪权限按功能拆分：`payment.selection.view`（选品收集）、`payment.records.view`（打款记录）、`payment.open`（开启打款）、`payment.stage_reopen`（阶段重开）、`payment.delete`（删除记录）。
- `payment.view.all` 用于查看全部店铺打款数据，并扩大到全部店铺作品审核范围；`payment.manage.all` 在此基础上拥有全店铺新增、编辑、删除、恢复、重开和审核能力。
- 店长审核由店长身份和店铺范围控制；超级管理员和子管理员可审核全部店铺记录。

## 打款跟踪

打款跟踪记录以商品为单位，按以下阶段推进：

```text
信息及选品 → 第二阶段 → 第三阶段 → 总结阶段
```

- 选品记录可从运营美工作品审核中开启，也可按权限手动新增。
- 开启打款时自动带入来源任务款号、主图和任务发布时间。
- 每个阶段支持保存、进入下一阶段、结束流程、恢复流程和历史阶段重开。
- 打款记录以时间线展示阶段进度、结束位置和结束原因；后续阶段在流程未满足条件时不可进入。
- 第二、第三阶段支持链接状态记录；图片按阶段独立保存，支持多图上传、预览、排序、删除、拖拽和截图粘贴。
- 非店长用户提交第一阶段后，可向同店铺店长发起付费审核；店长可查看、通过或拒绝。
...
系统文件分为参考文件、作品文件、完成凭证、打款跟踪图片和用户头像等类型，记录保存在对应数据表中，真实文件存储在配置目录中。
...
实际目录可通过系统配置和环境变量调整，打款跟踪图片和用户头像目录也可由超级管理员配置。上传文件会进行路径解析和类型处理，避免直接暴露任意本地路径。
...
相对 15.2.8，15.2.9 主要包括：

- 新增打款跟踪分区，支持选品收集、四阶段流程、时间线记录、恢复和阶段重开。
- 新增运营美工作品审核中的开启打款和批量开启打款，并自动带入来源任务信息。
- 新增店长身份和店长付费审核流程，支持查看、通过和拒绝。
- 新增全店铺打款数据查看、管理和跨店铺作品审核范围权限。
- 新增推广方式、上架类目、头像存储目录等超级管理员配置项。
- 新增打款跟踪图片、链接优化图片和数据反馈图片的多图上传、预览、删除、拖拽及截图粘贴。
- 新增用户头像修改、圆形裁剪和即时同步。
- 新增运营美工全量任务参考图/作品预览及款号搜索。
- 新增高级美工项目类型完成统计时间筛选和美工个人月度项目类型完成统计。
- 修复打款恢复、阶段重开、图片命名、上传弹窗层级、详情页字段混用和服务器重启后配置丢失等问题。
- 统一优化各角色任务详情页布局，保持原有任务流转、审核、上传和下载逻辑不变。
