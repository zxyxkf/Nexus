<template>
  <main class="payment-page">
    <header class="page-header">
      <div>
        <h1>选品收集</h1>
        <span class="record-count">{{ total }} 条进行中</span>
      </div>
      <el-button type="primary" :icon="Plus" :loading="creating" @click="createRecord">新增记录</el-button>
    </header>

    <section class="filter-bar" aria-label="选品筛选">
      <el-input
      ...
      <el-select v-model="filters.stageCode" clearable placeholder="全部阶段" @change="applyFilters">
    ...
    <section v-loading="loading" class="record-list">
      <ProductRowCard
        ...
        :key="record.id"
        :record="record"
        @continue="openStage(record, record.currentStage)"
...
import { createPaymentRecordApi, deletePaymentRecordApi, listPaymentRecordsApi } from '@/api'
...
async function loadRecords() {
  ...
  try {
    ...
    console.error('[PaymentTracking] 加载选品记录失败:', error)
  } finally {
    loading.value = false
  }
}
...
async function createRecord() {
  creating.value = true
  try {
    const response = await createPaymentRecordApi({})
    if (response.code !== 0) return
    await router.push(`/payment-tracking/records/${response.data.id}/stages/selection`)
  } finally {
...
async function deleteRecord(record) {
  try {
    await ElMessageBox.confirm(`确认删除货号“${record.styleNumber || '未填写'}”的选品记录？`, '删除确认', {
