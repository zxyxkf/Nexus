...
function normalizeManifest(manifest) {
  ...
  const normalized = manifest.map(item => {
    ...
    if (editedOriginalName.length > 500) throw new AppError(400, '编辑图片文件名无效');
...
async function persistTaskStyleSnapshots({ conn, task, taskId, materialStyleId, manifest, files, user, writtenPaths = [] }) {
  ...
  if (Number(lockedTask.publisher_id) !== Number(user.id) && user.role !== 'admin') {
  ...
  const replacedPaths = oldFiles.map(file => file.file_path).filter(Boolean);
  await conn.execute(
  ...
  for (const item of orderedManifest) {
    ...
    if (editedFile) {
      buffer = editedFile.buffer || fs.readFileSync(editedFile.path);
      fileName = safeDisplayName(item.editedOriginalName || editedFile.originalname, `material-${item.materialImageId}-效果图.png`);
      mimeType = 'image/png';
      extension = '.png';
    } else {
...
async function saveTaskStyleSnapshots({ taskId, materialStyleId, manifest, files, user }) {
  ...
  try {
    await executeTransaction(async conn => {
      const task = await taskDao.getTaskForUpdate(conn, Number(taskId));
      publisherId = task?.publisher_id || null;
      result = await persistTaskStyleSnapshots({
        conn, task, taskId, materialStyleId, manifest, files, user, writtenPaths
      });
    });
  } catch (error) {
  ...
  result.replacedPaths.forEach(removePhysicalFile);
  if (global.io) {
...
module.exports = { saveTaskStyleSnapshots, persistTaskStyleSnapshots, normalizeManifest };
